...
var fetchOne = function(nfo, prefix, title, revid) {
	// xxx prefix
	var api = new Api(nfo);
	var q = {
		action: 'query',
	...
	return api.request(q).then(function(resp) {
...
var fetchAndWrite = function(nfo, titles, outdir, log) {
	...
	titles.forEach(function(t) {
		p = p.then(function() {
			...
			return fetchOne(nfo, t.prefix, t.title, t.revid);
		}).then(function(data) {
			return writeOne(data, revStream, revDb);
		});
	});
	p = p.then(function() {
		return revDb.close();
