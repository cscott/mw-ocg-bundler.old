...
module.exports.process = function (parsoid, metabook, authorsDb, imageDb, status) {
	...
	var parseWikitext = function() {
		...
		var wikitext = util.format(
			...
			mangle(metabook.mw_rights_text || metabook.mw_license_url || '')
		);
		// XXX using the first wiki here is probably not correct; we
		// should respect the metabook.lang setting to choose an
		// appropriate prefix/wiki id.
