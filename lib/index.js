...
module.exports.bundle = function(metabook, options) {
	...
	var status = options.status = new StatusReporter(5, function(msg) {
		...
		log('['+msg.percent.toFixed()+'%]', msg.status + file);
	});
	...
	var fetchParsed = function() {
		...
		var doOneItem = function(item) {
			...
			return parsoid.fetch(item.wiki, item.title, item.revision, status)
				.then(function(result) {
					...
					return html.fetch(item.wiki, item.title, item.revision, status);
				}).then(function(result) {
	...
	var fetchImages = function() {
		status.createStage(imageMap.size, 'Fetching media');
	...
	var fetchRevisions = function() {
		...
		return Revisions.fetchAndWrite(
			metabook.wikis, titles, options.output, status, PEDIAPRESS_COMPAT
		);
	};
