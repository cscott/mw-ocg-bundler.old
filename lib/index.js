...
module.exports.bundle = function(metabook, nfo, options) {
	...
	var parsoid = new Parsoid(nfo, log);
	var html = new Html(nfo, log);
	...
	var fetchParsed = function() {
		...
		var doOneItem = function(item) {
			var prefix = 'en'; // XXX!
			return parsoid.fetch(prefix, item.title, item.revision)
				.then(function(result) {
					var revid = result.getRevisionId();
					if (!revid) { throw new Error("No revision ID"); }
					item.revision = '' + revid;
					sourceMap.set(prefix+':'+item.title, result);
					result.getImages().forEach(function(img) {
						imageMap.set(img.resource, img);
					});
					return parsoidDb.put(item.revision, result.text);
				}).then(function() {
					return html.fetch(prefix, item.title, item.revision);
				}).then(function(result) {
					return htmlDb.put(item.revision, JSON.stringify(result));
				});
		};
	...
	var fetchImages = function() {
		...
		imageMap.forEach(function(img) {
			var p = Image.fetch(img.resource, img.src, imagedir, log)
				.then(function(name) {
					img.filename = name;
				}).then(function() {
					// xxx download metadata
					var metadata = {
						height: 0, // XXX
						width: 0, // XXX
						thumburl: img.src,
						descriptionurl: img.resource,
						filename: img.filename // our extension
					};
					return imageDb.put(img.resource,
	...
	when.resolve()
		.then(Siteinfo.fetchAndWrite.bind(Siteinfo, nfo, options.output))
		.then(fetchParsed)
		.then(mkImageDir)
		.then(fetchImages)
